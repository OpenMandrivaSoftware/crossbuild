...
if echo $SUFFIX |grep -q x32; then
	GCCEXTRAARGS="$GCCEXTRAARGS --with-multilib-list=mx32"
fi
...
-musl*)
	LIBC=musl
	# FIXME libstdc++ vs. libcxx should be a separate option,
	# not tied to libc
	GCCEXTRAARGS="$GCCEXTRAARGS --disable-libstdc__-v3"
	;;
esac
...
if [ "$TOOLCHAIN_DONE" != "true" ]; then
	...
	../configure --prefix=/usr --target=$TARGET --with-sysroot=$SYSROOT $GOLDFLAGS --enable-plugins --enable-threads --enable-initfini-array
	make $SMP_MFLAGS
	sudo make install
	cd ../../../..
	...
	../configure --prefix=/usr --target=$TARGET --with-sysroot=$SYSROOT --disable-multilib --disable-__cxa_atexit --disable-libmudflap --disable-libssp --disable-threads --disable-tls --disable-decimal-float --disable-libgomp --disable-shared --disable-libstdc__-v3 --disable-sjlj-exceptions --disable-libitm --disable-libquadmath --disable-libvtv --disable-libatomic --disable-libcilkrts --disable-libsanitizer --enable-languages=c,c++ --with-newlib $GCCEXTRAARGS
	make $SMP_MFLAGS
	sudo make install
	cd ../../../..
	...
	musl)
		...
		CROSS_COMPILE=$TARGET- CC=/usr/bin/$TARGET-gcc ./configure \
			...
			--enable-shared \
			--enable-static
		make $SMP_MFLAGS CROSS_COMPILE=$TARGET- CC=/usr/bin/$TARGET-gcc
	...
	# musl doesn't need a stage2 compiler -- it can build with the
	# stage1 compiler just fine.
	if [ "$LIBC" != "musl" ]; then
		echo ========================================================================
		...
		cd build-stage2
		../configure --prefix=$USR --target=$TARGET --with-sysroot=$SYSROOT --disable-libssp --disable-libgomp --disable-libmudflap --disable-libquadmath --disable-libvtv --disable-libatomic --disable-libcilkrts --disable-libsanitizer --enable-languages=c --disable-multilib $GCCEXTRAARGS
		make $SMP_MFLAGS
	...
	cd build-stage3
	../configure --target=$TARGET --prefix=$USR --with-sysroot=$SYSROOT --enable-__cxa_atexit --disable-libssp --disable-libmudflap --disable-libquadmath --disable-multilib --disable-libgomp --enable-languages=c,c++,objc,obj-c++,lto $GCCEXTRAARGS
	make $SMP_MFLAGS
	sudo make $SMP_MFLAGS install
	sudo cp -a $TARGET/libgcc/libgcc_s* $SYSROOT$USR/lib
	cd ../../../..
